...
ask_questions() {
  ...
  echo ""
  echo "The Traefik prod config (infra/traefik/traefik.prod.yml) is already"
  echo "in the repo — no generation needed."
  echo ""
...
with open('infra/keycloak/realm-config.json') as f:
    realm = json.load(f)

domain = '${DOMAIN}'
gateway = '${GATEWAY_HOSTNAME}'

# Add production redirect URIs to pwa-frontend client
for client in realm.get('clients', []):
    if client.get('clientId') == 'pwa-frontend':
        ...
        print(f'  Added {len(new_uris)} redirect URIs, {len(new_origins)} web origins')
        break
else:
    print('  WARNING: pwa-frontend client not found in realm config')

with open('infra/keycloak/realm-config.prod.json', 'w') as f:
    json.dump(realm, f, indent=2)
" || error "Failed to generate realm-config.prod.json"

  success "infra/keycloak/realm-config.prod.json written."
}
